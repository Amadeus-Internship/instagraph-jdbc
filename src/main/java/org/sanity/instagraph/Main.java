...
import org.sanity.instagraph.data.dao.impl.*;
...
import org.sanity.instagraph.data.procedures.impl.ProcedureManagerImpl;
    ...
    public static Map<String, Executable> initializeCommands() {
        ...
        try {
            ...
            commands.put("update", (args) -> {
                new UsersDaoImpl()
                        .executeBlank("UPDATE users AS u " +
            ...
            System.out.println("Users are already updated!");
        }

        try {

            commands.put("delete", (args) -> {
                new UsersDaoImpl()
                        .executeBlank("DELETE u FROM users AS u " +
                                "LEFT JOIN users_followers AS uf ON u.id = uf.user_id  " +
                                "WHERE uf.user_id IS NULL;");
            });
        } catch (Exception e) {
            System.out.println("Users are already deleted!");
        }

        commands.put("query-users", (args) -> new UsersDaoImpl()
                .getUsers()
                .forEach(System.out::println));

        commands.put("query-cheaters",
                (args) -> new UsersDaoImpl()
                        .getCheaters()
                        .forEach(System.out::println));

        commands.put("query-high-quality-pictures",
                (args) -> new PicturesDaoImpl()
                        .getHighQualityPictures()
                        .forEach(System.out::println));

        commands.put("query-comments-and-users",
                (args) -> new CommentsDaoImpl()
                        .getCommentsAndUsers()
                        .forEach(System.out::println));

        commands.put("query-profile-pictures", (args) -> new UsersDaoImpl()
                .getProfilePictures()
                .forEach(System.out::println));

        commands.put("query-spam-posts",
                (args) -> new PostsDaoImpl()
                        .getSpamPosts()
                        .forEach(System.out::println));

        commands.put("query-most-popular-user",
                (args) -> System.out.println(new UsersDaoImpl()
                        .getMostPopularUser()));

        commands.put("query-commenting-myself",
                (args) -> new UsersDaoImpl()
                        .getCommentingMyself()
                        .forEach(System.out::println));

        commands.put("query-user-top-posts",
                (args) -> new UsersDaoImpl()
                        .getUserTopPosts()
                        .forEach(System.out::println));

        commands.put("query-posts-and-commentators",
                (args) -> new PostsDaoImpl()
                        .getPostsAndCommentators()
                        .forEach(System.out::println));

        commands.put("init-procedure-post",
                (arguments) -> new ProcedureManagerImpl(new CoreDaoImpl()).definePostProcedure());

        commands.put("init-procedure-filter",
                (arguments) -> new ProcedureManagerImpl(new CoreDaoImpl()).defineFilterProcedure());

        commands.put("exec-procedure-post", (arguments) -> {
            PostProcedureCallDto postProcedureCallModel = new PostProcedureCallDto(){{
                 setUsername(arguments.get(0));
                 setPassword(arguments.get(1));
                 setCaption(arguments.get(2));
                 setPath(arguments.get(3));
             }};
    ...
    public static void main(String[] args) {
        ...
        while (!(inputLine = scanner.nextLine()).equals("exit")) {
            ...
            if (commands.containsKey(command)) {
                commands.get(command).execute(Arrays.stream(tokens).skip(1).collect(Collectors.toList()));
                System.out.println("Executed " + tokens[0] + " command");
            }
        }
    }
